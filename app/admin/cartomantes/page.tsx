...
export default async function CartomantesApprovalPage() {
    ...
    // Fetch pending cartomantes
    const { data: pendingCartomantes } = await supabase
        .from('profiles')
        .select('*')
        .eq('role', 'CARTOMANTE')
                    ...
                    pendingCartomantes?.map((profile) => (
                        <div key={profile.id} className="bg-[#12121a] border border-white/10 rounded-xl p-6 flex items-start justify-between">
                            ...
                            <div className="flex flex-col gap-2 shrink-0">
                                <form action={async () => {
                                    'use server'
                                    await updateCartomanteStatus(profile.id, 'APPROVED')
                                }}>
                                ...
                                <form action={async () => {
                                    'use server'
                                    await updateCartomanteStatus(profile.id, 'REJECTED')
                                }}>
