...
import {
    ...
    CreditCard, Eye, MessageSquare, Sparkles, AlertCircle
} from 'lucide-react'
...
export default async function SubscriptionsPage() {
    ...
    if (!user) {
        redirect('/login?next=/dashboard/cartomante/assinaturas')
    }
    ...
    // Fetch subscriptions for this reader
    ...
    const cancelledSubs = allSubs.filter(s => s.status === 'CANCELLED' || s.status === 'EXPIRED')

    // Fetch client details
    const clientIds = [...new Set(allSubs.map(s => s.client_id))]
    ...
    // Fetch gig details
    const gigIds = [...new Set(allSubs.map(s => s.gig_id).filter(Boolean))]
    const gigDetails: Record<string, { title: string }> = {}
    if (gigIds.length > 0) {
                                ...
                                {activeSubs.filter(s => new Date(s.next_reading_due) <= new Date()).map(sub => {
                                    ...
                                    return (
                                        <div key={sub.id} className="flex items-center gap-4 p-3 rounded-xl bg-white/5 border border-white/5">
                                            ...
                                            <Link href={`/dashboard/cartomante/mensagens?clientId=${sub.client_id}`}>
                        ...
                        {allSubs.length === 0 ? (
                            ...
                            allSubs.map(sub => {
                                ...
                                return (
                                    <div
                                        key={sub.id}
                                        className="rounded-[1.5rem] border border-white/5 bg-card-item hover:border-purple-500/20 hover:bg-white/5 transition-all p-5 flex flex-col md:flex-row items-start md:items-center gap-5"
                                    >
                                        {/* Client Avatar */}
