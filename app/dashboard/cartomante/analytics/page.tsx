...
export default async function AnalyticsPage() {
    ...
    if (!user) {
        redirect('/login?next=/dashboard/cartomante/analytics')
    }
    ...
    // Fetch reviews for this reader's orders
    const completedOrderIds = completedOrders.map(o => o.id)
    let allReviews: { rating: number; gig_id: string }[] = []
    if (completedOrderIds.length > 0) {
        const { data: reviews } = await supabase
            .from('reviews')
            .select('rating, gig_id')
    ...
    // Gig details for top performing
    ...
    if (gigIds.length > 0) {
        const { data: gigs } = await supabase
            ...
            .in('id', gigIds)
        if (gigs) gigs.forEach(g => { gigDetails[g.id] = { title: g.title, image_url: g.image_url } })
    }
    ...
    const topGigs = Object.entries(gigStats)
        .sort(([, a], [, b]) => b.revenue - a.revenue)
        .slice(0, 4)

    // Weekly earnings data (last 4 weeks) for chart
    ...
    // Rating distribution for bar visualization
    const ratingStars = [5, 4, 3, 2, 1]
    const ratingDist = ratingStars.map(s => allReviews.filter(r => Math.round(r.rating) === s).length)

    // Category distribution based on actual gig order counts
    const gigOrderCounts: Record<string, { title: string; count: number }> = {}
    completedOrders.forEach(o => {
        if (!gigOrderCounts[o.gig_id]) {
            gigOrderCounts[o.gig_id] = { title: gigDetails[o.gig_id]?.title || 'Serviço', count: 0 }
        }
    ...
    // Gig color palette for top performing
        ...
        <div className="min-h-screen bg-background-deep text-slate-200 font-sans selection:bg-purple-500/30 flex overflow-hidden">
            ...
            <main className="relative z-10 flex-1 h-screen overflow-y-auto scrollbar-hide pb-24 md:pb-8">
                <PageContainer className="px-4 md:px-8 py-6 md:py-12">
                        ...
                        <div className="p-6 rounded-2xl border border-white/10 bg-card-item relative">
                            ...
                            <div className="flex items-center gap-1">
                                {ratingDist.map((count, i) => (
                                    <div
                                        key={i}
                                        className={`h-2 rounded-full flex-1 ${i === 0 ? 'bg-green-500' :
                                            i === 1 ? 'bg-green-400' :
                                                i === 2 ? 'bg-amber-400' :
                                                    i === 3 ? 'bg-amber-500' : 'bg-red-400'
                                            }`}
                    ...
                    {/* ──── Middle Row: Earnings + Categories ──── */}
                        ...
                        <div className="p-6 rounded-2xl border border-white/10 bg-card-item">
                            ...
                            <div className="relative h-40">
                                <svg viewBox="0 0 400 150" className="w-full h-full" preserveAspectRatio="none">
                                    ...
                                    {/* Area fill */}
                                    <path
                                        d={`M0,${150 - (weeklyData[0] / maxWeekly) * 120} 
                                            C66,${150 - (weeklyData[0] / maxWeekly) * 120 - 20} 
                                             66,${150 - (weeklyData[1] / maxWeekly) * 120} 
                                             133,${150 - (weeklyData[1] / maxWeekly) * 120}
                                            C200,${150 - (weeklyData[1] / maxWeekly) * 120 + 15}
                                             200,${150 - (weeklyData[2] / maxWeekly) * 120}
                                             266,${150 - (weeklyData[2] / maxWeekly) * 120}
                                            C333,${150 - (weeklyData[2] / maxWeekly) * 120 - 10}
                                             333,${150 - (weeklyData[3] / maxWeekly) * 120}
                                             400,${150 - (weeklyData[3] / maxWeekly) * 120}
                                            L400,150 L0,150 Z`}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div className="p-6 rounded-2xl border border-white/10 bg-card-item">
                                ...
                                <div className="relative w-44 h-44">
                                    <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
                                        ...
                                        <circle
                                            ...
                                            strokeWidth="12"
                                        />
                                        {/* Returning segment (purple) */}
                                        <circle
                                            cx="50" cy="50" r="40"
                                            fill="none"
                                            stroke="rgb(139,92,246)"
                                            strokeWidth="12"
                                            strokeDasharray={`${retentionRate * 2.51} ${(100 - retentionRate) * 2.51}`}
                                            strokeLinecap="round"
                                        />
                                        {/* New segment (amber) */}
                                        <circle
                                            ...
                                            strokeDashoffset={`-${retentionRate * 2.51}`}
                                            strokeLinecap="round"
                                        />
                                    </svg>
                                    {/* Center text */}
                            ...
                            <div className="flex items-center justify-center gap-6">
                                <span className="flex items-center gap-2 text-xs text-slate-400">
                                    ...
                                    Recorrentes ({retentionRate}%)
                                </span>
                                <span className="flex items-center gap-2 text-xs text-slate-400">
                                    <span className="w-3 h-3 rounded-full bg-amber-500" />
                                    Novos ({100 - retentionRate}%)
                                </span>
                            </div>
                        </div>
                        ...
                        <div className="p-6 rounded-2xl border border-green-500/10 bg-card-item">
                            <div className="flex items-center justify-between mb-5">
                                ...
                                <button className="text-[10px] font-bold uppercase tracking-wider text-green-400 hover:text-green-300">
                                    Ver Detalhes
                                </button>
                            </div>
                                    ...
                                    topGigs.map(([gigId, stats], i) => {
                                        ...
                                        return (
                                            <div key={gigId} className="grid grid-cols-[1fr_60px_80px_60px] gap-3 items-center px-1 py-2.5 rounded-xl hover:bg-white/5 transition-colors">
                                                <div className="flex items-center gap-3 min-w-0">
                                                    <div className={`w-9 h-9 rounded-lg ${gigColors[i] || 'bg-indigo-500'} bg-opacity-20 flex items-center justify-center shrink-0 overflow-hidden`}>
                                                        {gig?.image_url ? (
                                                            ...
                                                            <Tag className={`w-4 h-4 ${i === 0 ? 'text-indigo-400' :
                                                                i === 1 ? 'text-amber-400' :
                                                                    i === 2 ? 'text-purple-400' : 'text-green-400'
                                                                }`} />
                                                        )}
                                                    </div>
                                                    <div className="min-w-0">
                                                        ...
                                                        <p className="text-[10px] text-slate-600">
                                                            {stats.impressions} imps • {stats.views} clics
                                                        </p>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="flex justify-center items-center gap-1 text-[10px] font-bold text-slate-400">
                                                    {convRate}%
                                                </div>
                                            </div>
                                        )
