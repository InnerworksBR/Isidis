...
export default async function ClientSubscriptionsPage() {
    ...
    if (!user) {
        redirect('/login?next=/dashboard/minhas-assinaturas')
    }
                    ...
                    <div className="space-y-4">
                        {allSubs.length === 0 ? (
                            <div className="rounded-[1.5rem] border border-white/5 bg-white/5 p-16 text-center backdrop-blur-md">
                                ...
                                <Link href="/cartomantes" className="inline-block mt-6">
                            ...
                            allSubs.map(sub => {
                                ...
                                const readerInitials = (reader?.full_name || 'C')
                                    .split(' ')
                                    .map((n: string) => n[0])
                                    .join('')
                                    .substring(0, 2)
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <h3 className="text-base font-bold text-white truncate mb-1">
                                                {gig?.title || 'Assinatura'}
                                            </h3>

                                            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-400">
                                                <span className="flex items-center gap-1.5">
                                                    <Sparkles className="w-3 h-3 text-indigo-400" />
                                                    Com {reader?.full_name || 'Cartomante'}
                                                </span>
                                                <span className="flex items-center gap-1.5">
                                        ...
                                        <div className="flex flex-col sm:flex-row items-center gap-2 w-full md:w-auto justify-end mt-4 md:mt-0 pt-4 md:pt-0 border-t border-white/5 md:border-t-0">
                                            <Link href={`/dashboard/mensagens?readerId=${sub.reader_id}`} className="w-full sm:w-auto">
