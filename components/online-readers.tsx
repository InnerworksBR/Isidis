...
export function OnlineReaders({ initialReaders }: { initialReaders: Profile[] }) {
    const { onlineUsers } = usePresence()

    // Filtra as cartomantes que estão online e pega até 4
    const onlineReaders = initialReaders.filter(reader => onlineUsers.has(reader.id)).slice(0, 4)
